...
Abbiamo gestito il progetto attraverso Github. Questa è la repo. <https://github.com/arct0r/R-Artigiani>
...
Le recensioni provengono da due fonti: places e twitter. Le lingue sono molteplici e i negozi di elettronica (Player) sono 3.
...
Essendo le recensioni in italiano la stragrande maggioranza decidiamo di prendere in considerazione solo quelle e quindi eliminiamo le altre. Questo è il nostro dataset dopo questo preprocessing:
...
Per prepararci all'analisi dei testi creiamo il corpus e la DFM togliendo parole "fastidiose" (#hashtags, @usernames) provenienti dai tweets.
...
Durante la fase di recensione manuale del sentiment ci siamo accorti che la maggiorparte dei text proveniva da twitter. Questi tweets erano quasi tutti neutrali e non ci è sembrato che avessero particolare utilità.
...
#library(EBImage)
#fig <- plot_ly(type="image", z=readImage('RFPlot.png')*255)
#fig
```
...
Abbiamo allenato 3 diversi algoritmi sul nostro training data con il sentiment manuale. Bayes, Random Forest e Support Vector Machine. Questo è un sample del loro risultato. In seguito analizzeremo piu' a fondo le loro performances attraverso il processo di cross validation.
...
Wow! Che sta succedendo qui? I nostri algoritmi stanno dando valutazioni completamente diverse! Quale sarà il migliore?

## 2.3 Cross validation

Per la cross validation abbiamo scelto un k=5.
...
è arrivata l'ora di testare la star, Naive Bayes. Come si comporterà il nostro eroe?
...
Nonostante abbiamo diminuito il numero di trees l'accuracy risulta inferiore rispetto a Bayes.
...
Ci risulta l`utilizzo di 173 support vectors. L`accuracy del support Vector Machine si posiziona tra Bayes e Random Forest.
...
Abbiamo determinato quale fosse l'algoritmo piu' performante basandoci sull'accuratezza e su f1. Bayes risulta il piu' performante.
...
è un test che ci siamo inventati per verificare le performances degli algoritmi sul testing data. Grazie alla cross validation sappiamo come gli algoritmi si comportano sul training data. Ma non sappiamo nulla della loro accuracy sul testing data. Dunque ci siamo inventati l'AAP (Actual Algorythm Performances).
...
kbl(AAP_test_data[1:10, ], longtable = T, booktabs = T, 
    caption = "Dataset Negozi di Elettronica, Recensioni") %>%
    kable_styling(c("bordered", "condensed", "hover"), 
                  full_width = F, font_size = 11) %>%
    row_spec(0, color = "black", bold = T, background = "#b8daba") %>%
...
Rispetto alla cross validation Bayes rimane il piu' performante ma ha comunque registrato un calo di prestazioni. Essendo la stragrande maggioranza dei tweets inutili sarebbe interessante capire come il quantitativo di questi nel campione influenza l'accuracy. La presenza dei tweet riduce o aumenta l'accuracy? Ci porteremo questo dubbio nella tomba.
...
Notiamo la presenza di emoji, tag, urls e altri elementi disturbanti per l'analisi.
...
Paragrafo che spieghi il grafico
...
Qualche commento su sto grafico

## 3.4 Newsmap

Iniziare con un paragrafo
